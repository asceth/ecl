...
@
{
        ...
        int n = prepare_cif(the_env, cif, return_type, arg_types, Cnil, cc_type,
                            &types);
        void *executable_closure;
        ffi_closure *closure = ffi_closure_alloc(sizeof(ffi_closure), &executable_closure);

        cl_object executable_closure_object = ecl_make_foreign_data(@':pointer-void',
                                                                    sizeof(ffi_closure),
                                                                    executable_closure);
        ...
        int status = ffi_prep_closure_loc(closure, cif, callback_executor,
                                          ECL_CONS_CDR(data), executable_closure);
